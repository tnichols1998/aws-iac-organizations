...
resource "aws_s3_bucket" "terraform_state" {
  bucket        = "terraform-state-aws-iac-organizations-qa"
  force_destroy = false

  tags = {
    Name        = "terraform-state-aws-iac-organizations-qa"
...
output "backend_config" {
  description = "Backend configuration for QA environment"
  value = {
...
# Create backend configuration file for QA environment
...
dynamodb_table = "${aws_dynamodb_table.terraform_state_lock.name}"
EOT
  filename = "${path.module}/../qa/backend-qa.conf"
