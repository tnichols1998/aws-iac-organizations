...
terraform {
  backend "s3" {
    ...
    # See backend-dev.conf for actual configuration
  }
}
...
# Optional: GitHub Actions OIDC (if enabled and not LocalStack)
module "github_oidc" {
  count = lookup(local.org_config, "github_actions", {}).enabled == true && !local.is_localstack ? 1 : 0
